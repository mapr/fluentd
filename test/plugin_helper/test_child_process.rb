...
class ChildProcessTest < Test::Unit::TestCase
  ...
  sub_test_case 'launches a command just once' do
    ...
    test 'can execute external command at just once, which finishes immediately' do
      ...
      Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
        ...
        Thread.pass until m.locked? || ran
        m.lock
        m.unlock
      end
    ...
    test 'can execute external command at just once, which can handle both of read and write' do
      ...
      Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
        ran = false
        @d.child_process_execute(:t2, "ruby -e 'while !STDIN.eof? && line = STDIN.readline; puts line.chomp; STDOUT.flush; end'") do |io|
          m.lock
          data = begin
                   io.read_nonblock(1)
    ...
    test 'can execute external command at just once, which runs forever' do
      ...
      Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
        ran = false
        @d.child_process_execute(:t2, "ruby -e 'while sleep 0.01; puts 1; STDOUT.flush; end'") do |io|
          m.lock
  ...
  test 'execute external processes only for writing' do
    ...
    Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
      ran = false
      @d.child_process_execute(:t7, "ruby", arguments: ['-e', 'a=""; while b=STDIN.readline; a+=b; end'], read: false, write: true) do |io|
        m.lock
        unreadable = false
        begin
          io.read
  ...
  test 'execute external processes only for reading' do
    ...
    Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
      ran = false
      @d.child_process_execute(:t8, "ruby", arguments: ['-e', 'while sleep 0.01; puts 1; STDOUT.flush; end'], read: true, write: false) do |io|
        m.lock
        unwritable = false
        begin
          io.write "foobar"
  ...
  unless Fluent.windows?
    test 'can specify subprocess name' do
      ...
      Timeout.timeout(TEST_DEADLOCK_TIMEOUT) do
        ran = false
        @d.child_process_execute(:t10, "ruby", arguments:['-e', 'sleep 10'], subprocess_name: "sleeper") do |io|
          m.lock
          pids << io.pid
        ...
        assert{ proc_lines.select{|line| line =~ /^\s*#{pid}\s/ }.first.split(/\s+/)[3] == "sleeper" }
        @d.stop; @d.shutdown; @d.close; @d.terminate
      end
    end
  end
